...
impl SparseMerkleTree {
    ...
    pub fn add_leaf(
        ...
        self.storage.store_node(0, index, &leaf_hash)?;

        self.update_path_to_root(0, index, &leaf_hash)?;
    ...
    pub fn get_proof(&self, index: u64) -> Result<SparseMerkleProof, Box<dyn std::error::Error>> {
        if index >= MAX_LEAVES {
            return Err("Index exceeds maximum tree capacity".into());
        }
    ...
    fn update_path_to_root(
        ...
        for level in start_level..self.depth {
            ...
            self.storage
                .store_node(parent_level, current_index, &parent_hash)?;
